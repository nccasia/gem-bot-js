...
class Grid {
  ...
  prioritySwapGem(listMatchGem) {
    ...
    if (listMatchGemModifier?.length > 1) {
      return listMatchGemModifier[
        this.randoomm(listMatchGemModifier.length - 1)
      ];
    }
    matchGem = !midGame
      ? listMatchGem.find(
  ...
  getMatchGemStartGame(listMatchTypes) {
    return (
      ...
      this.randoomm(listMatchTypes.length - 1)
    );
  }
  ...
  checkMatchSwapGem(listMatchGem, currentGem, swapGem) {
    ...
    if (matchGems.size > 0) {
      listMatchGem.push(
        new GemSwapInfo(
          listMatchGem?.length ? listMatchGem.length : 0,
          currentGem.index,
          swapGem.index,
          matchGems.size,
          currentGem.type,
  ...
  checkMatchSwapSword(listMatchGem, currentGem, swapGem) {}
